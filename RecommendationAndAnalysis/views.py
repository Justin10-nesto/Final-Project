...
def studentMarksPrediction(request):
    ...
    subject_mapping = {i.id:i.subject_name for i in subjects_obj}
    print('ok')
...
def studentRecomendation(request):
    student = Student.objects.filter(user = request.user)
    ...
    if student.exists():
        ...
        if request.method == 'POST':
            ...
            if current_class == 'Form One':
                ...
                with open(marks_model, 'rb') as file:
                    loaded_moddel = pickle.load(file)

                    # loaded_moddel = pickle.load(open(marks_model, 'rb'))
...
def studentAnalysisByExam(request):
    student = Student.objects.filter(user = request.user)
    student_exams = StudentExam.objects.filter(is_verified = True)
    student_details = {'subject':[], 'exam_type':[], 'class':[],
        'hours_of_studying':[], 'participation_in_group':[], 'group_parlagrims':[],
...
def StudentProgress(request, sid):
    student = Student.objects.filter(user = request.user)
    number_tutorials = 0
    number_assigments = 0
    submited_assigment =0
    number_exams = 0
    total_exams =3
    number_tutorials_completed = 0
    tutotials_percentage_arr = []
    
    if student.exists():
        subject = Subject.objects.filter(id = sid).first()
        exams = StudentExam.objects.filter(student = student.first())
        if exams.exists():
            previous_data = ''
            for exam in exams:
                current_type = exam.exam.name
                if current_type != previous_data:
                    number_exams +=1
                previous_data = current_type
                
        subject_class = SubjectClass.objects.filter(studentClass = student.first().classCurrent, subject = subject)
        if subject_class.exists():
            topics = Topic.objects.filter(subject = subject_class.first())
            if topics.exists():
                for topic in topics:
                        number_tutorials += Tutorial.objects.filter(topic = topic).count()
                        number_assigments += Assigment.objects.filter(topic = topic).count()
                        submited_assigment += AssigmentSubmission.objects.filter(topic = topic).count()
                        tutorials = Tutorial.objects.filter(topic = topic)
                        if tutorials.exists():
                            tutorials_per_topic = []
                            
                            for tutorial in tutorials:
                                watched_tutorial_obj =  TutorialTimeTacking.objects.filter(tutorial = tutorial, user = request.user)
                                if watched_tutorial_obj.exists():
                                    watched_tutorial = watched_tutorial_obj.first()
                                    number_tutorials_completed +=watched_tutorial_obj.count()
                                    watched_tutorial_percentage = watched_tutorial.video_complition_percentage                                    
                                    tutorials_per_topic.append(watched_tutorial_percentage)
                            if tutorials_per_topic:
                                avg_tutorials_per_topic = sum(tutorials_per_topic)/len(tutorials_per_topic)
                                tutotials_percentage_arr.append(avg_tutorials_per_topic)
    if tutotials_percentage_arr:
        avg_tutotials_percentage_arr = sum(tutotials_percentage_arr)/len(tutotials_percentage_arr)
    else:
        avg_tutotials_percentage_arr = 0
    if number_assigments !=0:
        assigment_complition_percentage = number_assigments/submited_assigment *100
    else:
        assigment_complition_percentage = 0
    perc_exams = number_exams/total_exams*100
    ...
    try:
        ...
        subjects = Subject.objects.all()
    context = {'subjects':subjects, 'number_tutorials':number_tutorials, 'number_assigments':number_assigments, 'assigment_complition_percentage':assigment_complition_percentage, 'submited_assigment':submited_assigment,'number_exams':number_exams, 'perc_exams':perc_exams, 'number_tutorials_completed':number_tutorials_completed, 'avg_tutotials_percentage_arr':avg_tutotials_percentage_arr}
